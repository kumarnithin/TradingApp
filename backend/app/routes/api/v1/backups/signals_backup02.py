"""
TradingView Signal Handler - Auto Trading Integration (FINAL FIX)
Receives webhooks from TradingView alerts and auto-executes trades on IB
Fixed: Proper response handling and error checking
"""
...
class SignalData(BaseModel):
    ...
    exchange: Optional[str] = None  # Will default to SMART
...
async def receive_tradingview_signal(signal: SignalData):
    ...
    FINAL FIX: Proper error handling and response validation
    """
    try:
        ...
        if signal_config.auto_execute and signal_config.enabled:
            ...
            # FIXED: Use SMART exchange instead of ISLAND
            # SMART routing lets IB choose the best exchange to avoid direct routing fees
            exchange = "SMART"  # Always use SMART to avoid ISLAND direct routing issues
            
            if signal.contract_type.lower() == "stock":
                exchange = "SMART"  # ✅ Use SMART for stocks
            elif signal.contract_type.lower() == "forex":
                exchange = None  # Forex doesn't use exchange
            elif signal.contract_type.lower() == "future":
                exchange = "CME"  # Futures typically use CME
            elif signal.contract_type.lower() == "crypto":
                exchange = "PAXOS"  # Crypto uses PAXOS
            ...
            result = await ib_client.place_order(
                ...
                exchange=exchange  # Use SMART to avoid direct routing
            )
            
            # FIXED: Check status correctly - don't return error message if successful
            if result and result.get("status") == "success":
                ...
                # FIXED: Get actual error message instead of None
